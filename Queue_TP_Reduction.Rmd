...
sim_tracing = function(capacity_ratio = 0.8,
  ...
  create_swab_date = function(n, mean_rate) {
    ...
    while (length(swab_date) < (n)) {
      ...
      # Size parameter multiplier can be taken from Queue_Parameters.Rmd
      swab_date = c(swab_date, rep(i, rnbinom(1, mu=mean_rate, size=negbin_size)))
...
negbin_sizes = c(1, 10, 100)
# scenarios = expand_grid(mean_rate=mean_rates,
#                         capacity_ratio=capacity_ratios,
#                         negbin_size=negbin_sizes,
...
strategy_samples = future_lapply(scenarios, function(s) {
  sim_tracing(mean_rate = s$mean_rate,
              ...
              days_samples = 6000, # change this one to increase sample size in the results. 1000 is reasonably quick. 10000 encounters scaling issues.
              f_priority = strategies[[s$strategy]],
              negbin_size = s$negbin_size) %>%
...
strategy_plot_data = bind_rows(strategy_summary) %>%
  ...
  filter(negbin_size==10) # select one of these for results, use the rest in sensitivity analysis
...
ggplot(strategy_plot_data,
  ...
  labs(title = "Interview delay distributions for prioritisation strategies",
       x = "Days delayed from notification to interview",
       y = "Proportion of cases",
       fill = "Interviewed\n(to show NAs)")
ggsave("plot2.png", width=150, height=200, units="mm")
```
...
strategy_tp = bind_rows(strategy_tp) %>%
  mutate(capacity_ratio_label = percent(capacity_ratio, suffix="% capacity"),
         size_label = paste0("size=", negbin_size),
         strategy = strategy %>% str_replace(" (?!.* )", "\n") %>% fct_inorder())

ggplot(strategy_tp %>%
         filter(negbin_size==10), aes(x=strategy, y=mean, label=percent_format(accuracy=1)(mean))) +
...
ggplot(strategy_tp, aes(x=strategy, y=mean, label=percent_format(accuracy=1)(mean))) +
  ...
  theme(strip.background.x = element_rect(fill="grey80", color=NA),
        ...
        panel.grid.minor.y = element_blank(),
        axis.text.x = element_text(angle=90, vjust=0.5, hjust=1))
ggsave("plot3_appendix.svg", width=150, height=150, units="mm")
```
