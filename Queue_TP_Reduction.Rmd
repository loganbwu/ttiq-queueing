...
sim_tracing = function(capacity_ratio = 0.8,
  ...
  for (sim_day in seq_len(max(samples$notification_date))) {
    ...
    samples = samples %>%
      mutate(eligible_for_interview = notification_date <= sim_day & # Notification has happened
               ...
               swab_date >= (sim_day-max_interview_delay) & # Notification hasn't expired
  ...
  samples = samples[(n_burnin+1):(n_burnin+n_samples),] %>%
    mutate(
      ...
      time_to_interview = replace_na(interview_date - swab_date, Inf)
    )
}
```
...
strategy_samples = future_lapply(strategies, function(f) {
...
strategy_isolation = lapply(strategy_samples, function(df) {
...
ggplot(strategy_tp, aes(x=name, y=mean, label=round(mean, 2))) +
  geom_col(fill = "steelblue") +
  # geom_errorbar(aes(ymin=lower, ymax=upper), width=0.5) +
  geom_text(vjust = 1, color = "white", nudge_y = -0.005) +
  # coord_cartesian(ylim = c(min(strategy_tp$mean - 0.05), NA)) +
  coord_cartesian(ylim = c(0, NA)) +
  labs(title = "Transmission reduction",
